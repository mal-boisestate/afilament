...
def run_intensity(actin_intensity_thr):
    ...
    config.norm_th_actin = actin_intensity_thr
    ...
    if RECALCULATE:
        ...
        for img_num in img_nums:
            try:
                ...
                image_data_path = os.path.join(config.imgs_objects, "image_data_" + str(img_num) + "_actin_thr_"
                                               + str(actin_intensity_thr)+".pickle")
...
if __name__ == '__main__':
    ...
    actin_thresholds = [40000, 30000, 20000, 10000, 5000]
    for thr in actin_thresholds:
        run_intensity(thr)
